# KYCResponse


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**id** | **number** |  | [optional] [default to undefined]
**customerId** | **number** |  | [optional] [default to undefined]
**customerName** | **string** |  | [optional] [default to undefined]
**kycLevel** | **string** |  | [optional] [default to undefined]
**status** | **string** |  | [optional] [default to undefined]
**assignedOfficer** | **string** |  | [optional] [default to undefined]
**riskScore** | **number** |  | [optional] [default to undefined]
**dueDate** | **string** |  | [optional] [default to undefined]
**completedAt** | **string** |  | [optional] [default to undefined]
**nextReviewDate** | **string** |  | [optional] [default to undefined]

## Example

```typescript
import { KYCResponse } from '@openapitools/openapi-typescript-axios';

const instance: KYCResponse = {
    id,
    customerId,
    customerName,
    kycLevel,
    status,
    assignedOfficer,
    riskScore,
    dueDate,
    completedAt,
    nextReviewDate,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
